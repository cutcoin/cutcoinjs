 ...
 * cutcoinjs/walletRPC
 *
 * A class for making calls to cutcoin-wallet-rpc using Node.js
 ...
 * @param {string} network - Cutcoin network to connect to (default: 'mainnet')
 *
 *   OR
 *
 ...
 *   OR
 *
 * @param {object} params - Use test: true to test if a wallet from cutcoin-wallet-rpc.json is available (running)
 *
 * @returns {promise} resolve, reject
 * @resolves {class} walletRPC
 */
class walletRPC {
  constructor(hostname = '127.0.0.1', port = 24250, user = undefined, pass = undefined, protocol = 'http', network = 'mainnet') {
    return new Promise((resolve, reject) => {
      this.wallets = require('./local-cutcoin-wallet-rpcs.json');
      ...
      if (typeof hostname == 'object') { // Parameters can be passed in as object/dictionary
        ...
        if ('autoconnect' in params) {
          if (params['autoconnect']) { // TODO add mainnet/testnet toggle
            ...
            .then(wallet => {
              this._run('get_balance') // This line is necessary in order to do the initial handshake between this wrapper and cutcoin-wallet-rpc; without it, the first request to the wrapper fails (subsequent request succeed, though.)
              .then(() => {
                resolve(this);
              })
              .catch(err => {
  ...
  /**
   * Test a local or remote Cutcoin wallet RPC API for accessibility
   *
   * @function _test
   * @param {string} hostname - RPC hostname
  ...
  _test(hostname = this.hostname, port = this.port, user = this.user, pass = this.pass, protocol = this.protocol) {
    ...
    return new Promise((resolve, reject) => {
      return axios.post(`${protocol}://${hostname}:${port}/json_rpc`, options)
      .then(result => {
        resolve({
          hostname: this.hostname,
          port: this.port,
  ...
  /**
   * Execute command on the cutcoin-wallet-rpc API
   *
   * @function _run
  ...
  _run(method = null, params = undefined) {
    ...
    return axios.post(`${this.protocol}://${this.hostname}:${this.port}/json_rpc`, options)
    .then(result => {
   ...
   *     {
   *       address_index: 0,
   *       address: 'cutb9vuXgqtS2fDGRRptm8QHdQb4NQv1ZWBDhUCzQAjYcCggvgTcvSsZX6WetiXw1p3HpTgAhTdmniSRwzqKNPDW4mUQt5FZsW',
   *       balance: 140000000000,
   *       unlocked_balance: 50000000000,
   ...
   *       base_address: "cutb9vuXgqtS2fDGRRptm8QHdQb4NQv1ZWBDhUCzQAjYcCggvgTcvSsZX6WetiXw1p3HpTgAhTdmniSRwzqKNPDW4mUQt5FZsW",
   *       label: "Primary account",
   *       tag: "",
   ...
   * @resolves {object} - Example: {
   *   account_index: 1,
   *   address: 'TCU1qyPcrNfS1mnbYB6C5FfTc4yJqLN4kL1t3P3Bkr7SMBRcBzdYKkDUFDSQTfMzjTXEBzqu29bEZR91PKbWxmzY3ah3X5SDJs'
   * }
   */
  ...
  /**
   * Send cutcoins.  Parameters can be passed in individually (as listed below) or as an object/dictionary (as listed at bottom.)
   ...
   * @param {number} ring_size - Number of outputs to mix in the transaction (optional)
   *
   *   OR
   *
   ...
   * @param {object} params - Array containing any of the options listed above, where only amount and address or a destionations array are required
   */
  transfer_split(amount, address = undefined, payment_id = undefined, get_tx_keys = false, mixin = 10, account_index = 0, subaddr_indices = undefined, priority = 2, unlock_time = undefined, do_not_relay = false, get_tx_hex = false, get_tx_metadata = false, ring_size = 11) {
    return new Promise((resolve, reject) => {
      let params = {};
      let destinations = [];
   ...
   * @param {string} token_name - Token name, 8 symbols max, capital letters of digits.
   * @param {number} token_supply - Supply of the token, 1 844 674 407 max.
   * @param {number} token_type - 1 - public supply (default), 2 - hidden supply. (optional)
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   fee: 86897600000,
   *   tx_blob: "",
   *   tx_id: "7663438de4f72b25a0e395b770ea9ecf7108cd2f0c4b75be0b14a103d3362be9"
   * }
   */
   create_token(token_name, token_supply, token_type = 1) {
    return new Promise((resolve, reject) => {
      let params = {};

      if (typeof token_name == 'object') { // Parameters passed in as object/dictionary
        params = token_name;

        if ('token_name' in params) {
          token_name = params['token_name'];
        } else {
          throw new Error('Error: Token name required');
        }
        if ('token_supply' in params) {
  ...
  /**
   * Send tokens. Parameters can be passed in individually (as listed below) or as an object/dictionary (as listed at bottom.)
   * If multiple destinations are required, use the object/dictionary (bottom) format and pass an array of objects containing recipient
   * addresses and amount in the destinations field, like destinations: [{amount: 1, token_id: 4705223981953712128, address: ...}, {amount: 2, token_id: 4705223981953712128, address: ...}]
   * Token IDs can be found by get_tokens command.
   *
   * @function transfer_token
   * @param {string} amount - Amount to transfer
   * @param {number} token_id - Amount to transfer
   * @param {string} address - Address to transfer to
   * @param {string} payment_id - Payment ID (optional)
   * @param {boolean} get_tx_key - Return transaction key after generation (optional)
   * @param {number} mixin - Mixin amount (ringize - 1) (optional)
   ...
   * }
   */
  transfer_token(amount, token_id = undefined, address = undefined, payment_id = undefined, get_tx_key = false, mixin = 10, account_index = 0, subaddr_indices = undefined, priority = 2, unlock_time = undefined, do_not_relay = false, get_tx_hex = false, get_tx_metadata = false, ring_size = 11) {
    return new Promise((resolve, reject) => {
      let params = {};
      let destinations = [];
      ...
      if (typeof amount == 'object') { // Parameters passed in as object/dictionary
        ...
        if ('destinations' in params) {
          ...
          if ('token_id' in params) {
            ...
            throw new Error('Error: Token ID required');
          }
          if ('address' in params) {
            address = params['address'];
          } else {
          ...
          destinations = [{ amount: new_amount, token_id: token_id, address: address }];
        }
        if ('payment_id' in params) {
          payment_id = params['payment_id'];
        }
      ...
      this._run('transfer_token', params)
      .then(result => {
        this.store() // Save wallet state after transfer
        .then(() => {
          resolve(result);
  ...
  /**
   * Look up wallet tokens balances
   *
   * @function token_balance
   * @param {number} token_id - Token ID (default: 0 = CUTCOIN)
   * @param {number} account_index - Index of account to look up (default: 0)
   * @param (array} address_indices - Array of subaddress indices to look up (optional)
   *
   * @returns {promise} resolve, reject
   ...
   * }
   */
   token_balance(token_id = 0, account_index = 0, address_indices = undefined) {
    let params = {
      token_id: token_id,
      account_index: account_index
    };
    ...
    return this._run('token_balance', params);
  }

  /**
   * Return list of all tokens present in blockchain.
   *
   * @function get_tokens
   * @param {string} prefix - Amount to transfer
   * 
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   tokens: [{
   ...
   * }
   */
   get_tokens(prefix = undefined) {
    let params = { prefix: prefix };
    return this._run('get_tokens', params);
  }
   ...
   *   standard_address: "cutb9vuXgqtS2fDGRRptm8QHdQb4NQv1ZWBDhUCzQAjYcCggvgTcvSsZX6WetiXw1p3HpTgAhTdmniSRwzqKNPDW4mUQt5FZsW"
   * }
   */
   ...
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   notes: ['cutcoinjs test suite transaction note']
   * }
   */
  get_tx_notes(txids) {
   ...
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   value: 'cutcoinjs unit test suite wallet attribute'
   * }
   */
  get_attribute(key) {
    let params = { key: key };
   ...
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   uri: '"cutcoin:TCU1qyPcrNfS1mnbYB6C5FfTc4yJqLN4kL1t3P3Bkr7SMBRcBzdYKkDUFDSQTfMzjTXEBzqu29bEZR91PKbWxmzY3ah3X5SDJs&tx_amount=1.000000000000&recipient_name=cutcoinjs%20testnet%20unit%20test%20suite%20wallet&tx_description=cutcoinjs%20unit%20test%20suite%20uri'
   * }
   */
   ...
   *     recipient_name: 'Cutcoin Project donation address',
   ...
   * @resolves {object} - Example: {}
   */
  create_wallet(filename = 'cutcoin_wallet', password = undefined, language = 'English') {
    let params = {
      filename: filename,
      password: password,
      language: language
   ...
   * @param {string} filename - Filename to use for new wallet (default: 'cutcoin_wallet')
   * @param {string} password - Password to use for new wallet (default: '')
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {}
   */
  open_wallet(filename = 'cutcoin_wallet', password = undefined) {
    let params = {
      filename: filename,
      password: password
    };
   ...
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   address: 'cut9xXa4CtQsHqaTBoAvZLgZAXjKkzQnXR5tTVtp6P6NwAj2hokhkDd8NmSyLZN95wXqGcpo2wS92KeRBxDQaikNzPJ9nujoUB',
   *   multisig_info: ''
   * }
   */
   ...
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   address: 'cut9v8xvJkkj9ZWLykFxh3X3v6w3nEgSVoHyXaWnd5zwz4h2jtUDk3enseWjq4BJNa8eBBqBVLwxj5KhfGhsMS8oDDHNbAULfc'
   * }
   */
...
function toAtomicUnits(num) {
  num = Math.floor(num * 10000000000);
